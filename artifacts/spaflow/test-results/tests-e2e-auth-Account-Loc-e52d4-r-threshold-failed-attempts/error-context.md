...
- Name: tests\e2e\auth.spec.ts >> Account Lockout >> should lock account after threshold failed attempts
- Location: tests\e2e\auth.spec.ts:172:3
...
```ts
  84  |             const usersData = await users.json();
  85  |             const existingUser = usersData.find((u: any) => u.email === LOCKOUT_TEST_USER.email);
  86  |             if (existingUser) {
  87  |               await request.delete(`http://localhost:5000/users/${existingUser.id}`, {
  88  |                 headers: {
  89  |                   'Cookie': cookies
  90  |                 }
  91  |               });
  ...
  93  |           }
  94  |         } catch (e) {
  95  |           // User might not exist, that's fine
  96  |         }
  97  |         
  98  |         // Create the test user for lockout testing
  99  |         await request.post('http://localhost:5000/users', {
  100 |           headers: {
  101 |             'Cookie': cookies
  102 |           },
  103 |           data: {
  104 |             email: LOCKOUT_TEST_USER.email,
  105 |             password: LOCKOUT_TEST_USER.password,
  106 |             name: LOCKOUT_TEST_USER.name,
  107 |             role: 'STAFF'
  108 |           }
  109 |         });
  110 |       }
  111 |     } catch (e) {
  112 |       // If setup fails, continue with test (user might already exist)
  113 |       console.log('Setup warning:', e);
  114 |     }
  115 |   });
  116 | 
