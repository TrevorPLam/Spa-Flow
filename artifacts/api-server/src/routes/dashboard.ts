...
router.get("/dashboard", requireAuth, apiLimiter, async (req, res): Promise<void> => {
  ...
  const [lockerStats, roomStats, revenueResult, activeClientsResult, waitlistCount, recentTxns, activeRentals, lowStockProducts] = await Promise.all([
    ...
    db.select().from(productsTable).where(sql`${productsTable.stock} <= ${productsTable.lowStockThreshold}`).orderBy(productsTable.name).limit(10),
  ]);
